...
export const POST = withAuth(['client'])(async (req) => {
  try {
    ...
    // Auto-criar comanda para este agendamento
    try {
      const { comandas, comandaItems } = await import('@/lib/db/schema');
      ...
      const countRes = await db
        ...
        .from(comandas)
        .where(and(eq(comandas.barbershopId, data.barbershopId), eq(comandas.referenceMonth, refMonth)));

      const seq = (countRes[0]?.count || 0) + 1;
      const code = `CMD-${ym}-${String(seq).padStart(4, '0')}`;

      const servicePrice = service ? parseFloat(service.price) : parseFloat(data.totalPrice || '0');
      const clientName = clientUser[0]?.name || 'Cliente';
      const barberName = barberUser[0]?.name || 'Barbeiro';
      ...
      console.error('Erro ao criar comanda (não bloqueia o agendamento):', comandaErr);
    }
